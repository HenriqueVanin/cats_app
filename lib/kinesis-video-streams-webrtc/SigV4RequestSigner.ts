...
export class SigV4RequestSigner implements RequestSigner {
    ...
    public async getSignedURL(
        ...
        const canonicalRequestHash = await SigV4RequestSigner.sha256(
            canonicalRequest
        );
        ...
        const signature = await SigV4RequestSigner.toHex(
            await SigV4RequestSigner.hmac(signingKey, stringToSign)
        );
    ...
    private async getSignatureKey(dateString: string): Promise<ArrayBuffer> {
        const kDate = await SigV4RequestSigner.hmac(
    ...
    private static async hmac(
        ...
        const keyBuffer =
            typeof key === 'string' ? this.toUint8Array(key).buffer : key;
