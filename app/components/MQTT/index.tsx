...
function useMQTT() {
    ...
    async function testSuccessfulConnection() {
        ...
        const suback = await client.subscribe({
            subscriptions: [
                { qos: mqtt5.QoS.AtLeastOnce, topicFilter: qos0Topic },
            ],
        });
    ...
    function initialSubscribeConfig() {
        Object.values(alertTopic).map((topic: string) =>
        ...
        if (client)
            client.on(
                ...
                (eventData: mqtt5.MessageReceivedEvent): void => {
                    ...
                    if (
                        Object.values(alertTopic).some(
                            (command) => command === eventTopic
                        )
                    ) {
