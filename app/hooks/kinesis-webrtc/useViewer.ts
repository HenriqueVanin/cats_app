...
import {
    Role,
    SignalingClient,
} from '../../../lib/kinesis-video-streams-webrtc';
    ...
    useEffect(() => {
        ...
        async function handleSignalingClientOpen() {
            logger.current.logViewer(
                `[${clientId.current}] signaling client opened`
            );
        ...
        async function handleSignalingClientSdpAnswer(answer) {
            logger.current.logViewer(
                `[${clientId.current}] received sdp answer`
            );
        ...
        function handleSignalingChannelIceCandidate(
        ...
        function handlePeerTrack({ streams = [] }) {
            logger.current.logViewer(
                `[${clientId.current}] received peer track`
            );
